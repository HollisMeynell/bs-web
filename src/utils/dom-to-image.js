...
function copyOptions(options) {
    ...
    if (typeof options.imagePlaceholder === 'undefined') {
        dom2image.impl.options.imagePlaceholder = defaultOptions.imagePlaceholder
    } else {
        dom2image.impl.options.imagePlaceholder = options.imagePlaceholder
    }

    if (typeof options.cacheBust === 'undefined') {
        dom2image.impl.options.cacheBust = defaultOptions.cacheBust
    } else {
        dom2image.impl.options.cacheBust = options.cacheBust
    }
}
...
function draw(domNode, options) {
    ...
    function newCanvas(domNode) {
        ...
        const scale = options.scale || 2 // 默然两倍大小
...
function newUtil() {
    ...
    function getAndEncode(url) {
        const TIMEOUT = 30000
        if (dom2image.impl.options.cacheBust) {
        ...
        return new Promise(function (resolve) {
            ...
            let placeholder
            if (dom2image.impl.options.imagePlaceholder) {
                const split = dom2image.impl.options.imagePlaceholder.split(/,/)
                if (split && split[1]) {
                    placeholder = split[1]
                }
            }
